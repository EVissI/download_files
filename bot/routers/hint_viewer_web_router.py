...
class HintFolderCreateBody(BaseModel):
    name: str = Field(min_length=1, max_length=255)
    parent_id: int | None = None


class HintFolderUpdateBody(BaseModel):
    folder_id: int
    name: str = Field(min_length=1, max_length=255)


class HintFolderIdBody(BaseModel):
    folder_id: int


class HintFolderItemsBody(BaseModel):
    folder_id: int
    upload_ids: list[int] = Field(default_factory=list)


def _serialize_hint_folder(
    folder: HintWebFolder, direct_files_count: int = 0
) -> dict[str, Any]:
    return {
        "id": folder.id,
        "name": folder.name,
...
def _build_hint_folder_tree(
    ...
    for f in folders:
        by_id[f.id] = {
            **_serialize_hint_folder(f, direct_counts.get(f.id, 0)),
            "children": [],
        }
    roots: list[dict] = []
    for f in folders:
...
async def web_hints_history(request: Request, page: int = 1, folder_id: int | None = None):
    ...
    if user_id and folder_id:
        async with async_session_maker() as db:
            ...
            folder = await folder_dao.get_folder_for_user(int(folder_id), int(user_id))
            if not folder:
                raise HTTPException(status_code=404, detail="Папка не найдена")
            scoped_folder_id = int(folder_id)
    if user_id:
...
@hint_viewer_web_api_router.post("/web/hints/api/folders/tree")
async def hint_folder_tree(request: Request):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    async with async_session_maker() as db:
        dao = HintWebFolderDAO(db)
        folders = await dao.get_all_folders(user_id)
        counts = await dao.get_direct_counts(user_id)
        return {"ok": True, "folders": _build_hint_folder_tree(folders, counts)}


@hint_viewer_web_api_router.get("/web/hints/api/folders/{folder_id}")
async def hint_folder_get(request: Request, folder_id: int):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    async with async_session_maker() as db:
        dao = HintWebFolderDAO(db)
        folder = await dao.get_folder_for_user(folder_id, user_id)
        if not folder:
            raise HTTPException(status_code=404, detail="Папка не найдена")
        counts = await dao.get_direct_counts(user_id)
        children = await dao.get_child_folders(folder_id, user_id)
        parent = None
        if folder.parent_id:
            parent_folder = await dao.get_folder_for_user(folder.parent_id, user_id)
            if parent_folder:
                parent = _serialize_hint_folder(
                    parent_folder, counts.get(parent_folder.id, 0)
                )
        return {
            "ok": True,
            "folder": _serialize_hint_folder(folder, counts.get(folder.id, 0)),
            "parent": parent,
            "child_folders": [
                _serialize_hint_folder(child, counts.get(child.id, 0))
                for child in children
            ],
        }


@hint_viewer_web_api_router.post("/web/hints/api/folders/create")
async def hint_folder_create(request: Request, body: HintFolderCreateBody):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    ...
    async with async_session_maker() as db:
        async with db.begin():
            ...
            return {"ok": True, "folder": _serialize_hint_folder(folder)}


@hint_viewer_web_api_router.post("/web/hints/api/folders/update")
async def hint_folder_update(request: Request, body: HintFolderUpdateBody):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    name = body.name.strip()
    if not name:
...
@hint_viewer_web_api_router.post("/web/hints/api/folders/delete")
async def hint_folder_delete(request: Request, body: HintFolderIdBody):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    async with async_session_maker() as db:
        async with db.begin():
            dao = HintWebFolderDAO(db)
            deleted = await dao.delete_folder(body.folder_id, user_id)
            if not deleted:
                raise HTTPException(status_code=404, detail="Папка не найдена")
            return {"ok": True}


@hint_viewer_web_api_router.post("/web/hints/api/folders/add_items")
async def hint_folder_add_items(request: Request, body: HintFolderItemsBody):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    async with async_session_maker() as db:
        async with db.begin():
            folder_dao = HintWebFolderDAO(db)
            folder = await folder_dao.get_folder_for_user(body.folder_id, user_id)
            if not folder:
                raise HTTPException(status_code=404, detail="Папка не найдена")
            upload_dao = HintViewerWebUploadDAO(db)
            owned_ids = await upload_dao.get_owned_upload_ids(
                user_id, body.upload_ids, service="hints"
            )
            if not owned_ids:
                raise HTTPException(status_code=404, detail="Файлы не найдены")
            added = await folder_dao.add_uploads_to_folder(body.folder_id, owned_ids)
            return {"ok": True, "added_count": added}


@hint_viewer_web_api_router.post("/web/hints/api/folders/remove_items")
async def hint_folder_remove_items(request: Request, body: HintFolderItemsBody):
    _token, session = await _require_session(request)
    user_id = _require_user_id(session)
    async with async_session_maker() as db:
        async with db.begin():
            ...
            for uid in owned_ids:
                if await folder_dao.remove_upload_from_folder(body.folder_id, uid):
                    removed += 1
