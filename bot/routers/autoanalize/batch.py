...
@batch_auto_analyze_router.callback_query(F.data == "autoanalyze_batch", UserInfo())
async def start_batch_auto_analyze(
    ...
    balance_match = await dao.get_total_analiz_balance(
        user_info.id, service_type=ServiceType.MATCH
    )
    balance_money = await dao.get_total_analiz_balance(
        user_info.id, service_type=ServiceType.MONEYGAME
    )
    if balance_match == 0 and balance_money == 0:
        await callback.message.answer(
            i18n.user.static.has_no_sub(),
            reply_markup=get_activate_promo_keyboard(i18n),
        )
        return
    keyboard = InlineKeyboardBuilder()
    keyboard.button(
        text=i18n.auto.batch.sequential(), callback_data="batch_type:sequential"
    )
    keyboard.button(text=i18n.auto.batch.zip(), callback_data="batch_type:zip")
    keyboard.adjust(1)
    await callback.message.answer(
        i18n.auto.batch.choose_type(), reply_markup=keyboard.as_markup()
    )


@batch_auto_analyze_router.callback_query(
    F.data.startswith("batch_type:"), StateFilter(BatchAnalyzeDialog.choose_type)
)
async def handle_batch_type_selection(
    ...
    if batch_type == "sequential":
        ...
        await callback.message.answer(
            i18n.auto.batch.submit_sequential(),
            reply_markup=keyboard.as_markup(resize_keyboard=True),
        )
    else:  # zip
        await state.set_state(BatchAnalyzeDialog.uploading_zip)
        await callback.message.answer(
            i18n.auto.batch.submit_zip(), reply_markup=get_cancel_kb(i18n)
        )
...
@batch_auto_analyze_router.message(
    F.text.in_(get_all_locales_for_key(translator_hub, "auto-batch-stop")),
    StateFilter(BatchAnalyzeDialog.uploading_sequential),
    UserInfo(),
)
async def handle_batch_stop(
    ...
    username = (
        user_info.player_username
        if user_info.player_username
        else f"user_{user_info.id}"
    )
        ...
        try:
            await message.bot.send_document(
                ...
                document=BufferedInputFile(
                    open(zip_path, "rb").read(), filename=os.path.basename(zip_path)
                ),
    ...
    await process_batch_files(
        message, state, user_info, i18n, file_paths, session_without_commit
    )
...
@batch_auto_analyze_router.message(
    ...
    StateFilter(
        BatchAnalyzeDialog.uploading_sequential, BatchAnalyzeDialog.uploading_zip
    ),
    UserInfo(),
)
    ...
    async with sequential_file_lock:
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to download file {file_name} for user {user_info.id}: {e}"
                )
            ...
            try:
                ...
                if file_path not in file_paths:
                    ...
                    logger.info(
                        f"Added file {file_path} for user {user_info.id}, total files: {len(file_paths)}"
                    )
                else:
                    logger.warning(
                        f"Duplicate file {file_path} skipped for user {user_info.id}"
                    )
            except Exception as e:
            ...
            logger.error(
                f"Unexpected error in handle_sequential_file for user {user_info.id}: {e}"
            )
            await message.answer(
...
async def handle_zip_file(
    ...
    if not file_paths:
        ...
        return await message.answer(
            i18n.auto.batch.no_valid_files(),
            reply_markup=MainKeyboard.build(user_info.role, i18n),
        )
...
async def process_batch_files(
    ...
    progress_message = await message.answer(
        i18n.auto.batch.progress(current=0, total=total)
    )
    data = await state.get_data()
    current_file_idx = data.get("current_file_idx", 1)
    for idx, file_path in enumerate(file_paths, current_file_idx):
        await message.bot.delete_message(
            chat_id=message.chat.id, message_id=progress_message.message_id
        )
        progress_message = await message.answer(
            i18n.auto.batch.progress(current=idx, total=total)
        )
        ...
        new_file_name = (
            f"{current_date}:{player_names[0]}:{player_names[1]}.{file_type}"
        )
        ...
        if user_info.player_username and user_info.player_username in player_names:
            ...
            # сохраняем вместе с именем файла
            all_analysis_datas.append(
                {"analysis": analysis_data, "file_name": new_file_name}
            )
            successful_count += 1
...
async def process_single_analysis(
    ...
    game_id = (
        f"batch_auto_{message.from_user.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    player_data = {
    ...
    if duration > 0:
        descrease_result = await user_dao.decrease_analiz_balance(
            user_info.id, ServiceType.MATCH
        )
        try:
    ...
    if duration == 0:
        descrease_result = await user_dao.decrease_analiz_balance(
            user_info.id, ServiceType.MONEYGAME
        )
    if descrease_result:
        data = await state.get_data()
        pr_values = data.get("pr_values", {})
        for player in players_metrics.keys():
            pr_values.setdefault(player, []).append(
                abs(players_metrics[player].get("snowie_error_rate", 0))
            )
...
@batch_auto_analyze_router.callback_query(
    F.data.startswith("batch_player:"),
    StateFilter(BatchAnalyzeDialog.select_player),
    UserInfo(),
)
    ...
    try:
        ...
        try:
            await callback.message.bot.delete_message(
                chat_id=callback.message.chat.id, message_id=progress_message_id
            )
        except:
            pass
        ...
        if process_result:
            for idx, file_path in enumerate(file_paths, current_file_idx + 1):
                ...
                if (
                    user_info.player_username
                    and user_info.player_username in player_names
                ):
                    file_paths = file_paths[1:]
                    await state.update_data(file_paths=file_paths)
                    selected_player = user_info.player_username
                    ...
                    for player in player_names:
                        keyboard.button(
                            text=player, callback_data=f"batch_player:{player}"
                        )
        ...
        try:
            await callback.message.bot.delete_message(
                chat_id=callback.message.chat.id, message_id=progress_message.message_id
            )
        except:
            pass
        await finalize_batch(
...
async def finalize_batch(
    ...
    if successful_count > 0:
        ...
        pr_values = data.get("pr_values", [])
        ru_i18n: TranslatorRunner = translator_hub.get_translator_by_locale("ru")
        group_pr_msg = (
            "<b>"
            + ru_i18n.auto.batch.summary_pr_header(
        ...
        players_order_str = (
            players_order_str
            + f"({datetime.now().strftime('%d.%m.%y')}_{datetime.now().strftime('%H;%M')}).pdf"
        )
        logger.info(players_order_str)
        await redis_client.set(
            f"pdf_file_name:{user_info.id}", players_order_str, expire=3600
        )
        await redis_client.set(f"user_pr_msg:{user_info.id}", user_pr_msg, expire=3600)
        ...
        try:
            ...
            for item in all_analysis_datas:
                ...
                pdf_pages.append(
                    make_page(
                        format_detailed_analysis(get_analysis_data(analysis), i18n), 11
                    )
                )
            ...
            await message.bot.send_document(
                chat_id=settings.CHAT_GROUP_ID,
                document=BufferedInputFile(pdf_bytes, filename=players_order_str),
...
@batch_auto_analyze_router.callback_query(
    DownloadPDFCallback.filter(F.context == "batch"), UserInfo()
)
async def handle_download_pdf(
    ...
    if callback_data.action == "yes":
        ...
        file_name = (
            file_name.replace(file_type, "pdf") if file_name else "batch_analysis.pdf"
        )
