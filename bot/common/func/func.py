...
def calculate_average_analysis(all_datas: list) -> dict:
    ...
    # Initialize aggregators for each player
    player_metrics = {}
    player_counts = {}  # To track number of valid entries per player
    
    # Numeric fields to average
    numeric_fields = [
        "moves_marked_bad",
        "moves_marked_very_bad",
        "error_rate_chequer",
        "rolls_marked_very_lucky",
        "rolls_marked_lucky",
        "rolls_marked_unlucky",
        "rolls_marked_very_unlucky",
        "rolls_rate_chequer",
        "missed_doubles_below_cp",
        "missed_doubles_above_cp",
        "wrong_doubles_below_sp",
    ...
    # Non-numeric fields to track (e.g., take most common)
    non_numeric_fields = [
        "chequerplay_rating",
        "luck_rating",
    ...
    for data in all_datas:
        formatted = get_analysis_data(data)  # Get dict with all players
        for player, metrics in formatted.items():
            if player not in player_metrics:
                player_metrics[player] = {field: [] for field in numeric_fields + non_numeric_fields}
                player_counts[player] = 0
            
            for field in numeric_fields:
                if field in metrics and isinstance(metrics[field], (int, float)):
                    player_metrics[player][field].append(metrics[field])
    ...
    # Calculate averages for numeric fields and most common for non-numeric
    result = {}
    for player in player_metrics:
        result[player] = {}
        for field in numeric_fields:
            values = player_metrics[player][field]
            result[player][field] = sum(values) / len(values) if values else 0
        
        for field in non_numeric_fields:
            values = player_metrics[player][field]
            if values:
                # Take most common value, or "N/A" if tied or empty
                from collections import Counter
                most_common = Counter(values).most_common(1)
                result[player][field] = most_common[0][0] if most_common else "N/A"
            else:
                result[player][field] = "N/A"
