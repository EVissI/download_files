...
async def expire_analiz_balances():
    try:
        async with async_session_maker() as session:
            ...
            for up in promos:
                ...
                if not promo or promo.duration_days is None:
                    continue
                expire_date = up.created_at
                if expire_date.tzinfo is None:
                    expire_date = expire_date.replace(tzinfo=timezone.utc)
                expire_date = expire_date + timedelta(days=promo.duration_days)
            ...
            for up in payments:
                ...
                if expire_date.tzinfo is None:
                    expire_date = expire_date.replace(tzinfo=timezone.utc)
                expire_date = expire_date + timedelta(days=payment.duration_days)
                if expire_date < now and up.is_active:
                    user = await user_dao._session.get(user_dao.model, up.user_id)
        ...
        logger.error(f"Error in expire_analiz_balances: {e}")