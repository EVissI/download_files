...
class BaseDAO(Generic[T]):
    ...
    async def update(self, data_id: int, values: dict | BaseModel):
        """
        Update record by id with given values.

        Args:
            data_id: ID of the record to update
            values: Dict or BaseModel with values to update
        ...
        logger.info(
            f"Обновление записи {self.model.__name__} с ID {data_id}, параметры: {values_dict}"
        )
        try:
            query = (
